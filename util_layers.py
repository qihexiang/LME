...
class AutoBondLayer:
    def __init__(self, radius_table) -> None:
        ...
        self.elements_table = py_.flatten(py_.map(elements, lambda e1: py_.map(elements, lambda e2: (e1, e2))))
        radiuses = py_.map(elements, lambda element: radius_table[element])
        self.bond_length_matrix = py_.flatten(py_.map(radiuses, lambda r1: py_.map(radiuses, lambda r2: r1 + r2)))
