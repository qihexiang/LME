...
class EditableLayer:
    ...
    def add_atom(self, new_atom):
        duplicated = py_.find(self.atoms, lambda atom: atom.get_id() == new_atom.get_id())
        if  duplicated is not None:
            raise ValueError(f"Duplicated atom id {new_atom.get_id()}.")
        
        too_closed = py_.find(self.atoms, lambda atom: np.linalg.norm(atom.position - new_atom.position) <= self.eps)
        if too_closed is not None:
            raise ValueError(f"Atoms too closed: exsited {too_closed}, new {new_atom}, eps: {self.eps}")
    ...
    def replace_atom(self, atom_id, element):
        ...
        def update(atoms, bonds):
            atoms = atoms[0:target] + [atoms[target].replace(element)] + atoms[target+1:]
            return atoms, bonds
        self.update_state(update)
    ...
    def get_bonds(self, from_atom):
        ...
        atom_pairs = py_.filter(atom_pairs, lambda atom_pair: atom_pair.has_atom(from_atom) and self.bonds[atom_pair] is not None)
        return atom_pairs
